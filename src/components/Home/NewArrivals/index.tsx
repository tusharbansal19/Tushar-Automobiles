...
import { fetchAllAutoParts } from "@/store/slices/autoPartsSlice";

const NewArrival = () => {
  const dispatch = useDispatch<AppDispatch>();
  const { allParts, loading, error } = useAppSelector((state) => state.autoPartsReducer);

  // Fetch all auto parts on component mount if not already loaded
  useEffect(() => {
    if (allParts.length === 0) {
      dispatch(fetchAllAutoParts());
    }
  }, [dispatch, allParts.length]);

  // Get the latest 8 parts sorted by creation date
  const latestParts = allParts
      ...
      <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
        ...
        {loading ? (
          ...
          <div className="text-center py-10">
            <p className="text-red-600 mb-4">Failed to load auto parts</p>
            <button 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-7.5 gap-y-9">
            {/* <!-- New Auto Parts item --> */}
            {latestParts.map((item, key) => (
              <ProductItem item={item as any} key={item._id || key} />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-7.5 gap-y-9">
            ...
            <div className="col-span-full text-center py-10">
              <p className="text-gray-5">No auto parts available</p>
            </div>
          </div>
        )}
      </div>
