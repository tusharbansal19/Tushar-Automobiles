...
import { AutoPart } from "@/store/slices/autoPartsSlice";
...
const SingleListItem = ({ item }: { item: AutoPart }) => {
    ...
    <div className="group rounded-lg bg-white shadow-1">
      <div className="flex">
        <div className="shadow-list relative overflow-hidden flex items-center justify-center max-w-[270px] w-full sm:min-h-[270px] p-4">
          <Image
            src={item.imgs?.previews?.[0] || item.imgs?.thumbnails?.[0] || "/images/placeholder-part.png"}
            alt={item.title}
            width={250}
            height={250}
          />
        ...
        <div className="w-full flex flex-col gap-5 sm:flex-row sm:items-center justify-center sm:justify-between py-5 px-4 sm:px-7.5 lg:pl-11 lg:pr-12">
          <div className="flex-1">
            <div className="mb-2">
              <p className="text-custom-sm text-gray-5 mb-1">{item.brand} • {item.company}</p>
              <p className="text-custom-xs text-blue bg-blue/10 px-2 py-1 rounded inline-block">
                {item.category}
              </p>
            </div>
            ...
            <div className="flex items-center gap-4 mb-2">
              ...
              <div className="flex items-center gap-1">
                <div className={`w-2 h-2 rounded-full ${
                  ...
                  item.stockStatus === 'limited-stock' ? 'bg-yellow-500' : 
                  item.stockStatus === 'pre-order' ? 'bg-blue-500' : 'bg-red-500'
                }`}></div>
            ...
            {item.specifications && Object.keys(item.specifications).length > 0 && (
              <div className="mt-3">
                <p className="text-custom-sm font-medium text-gray-6 mb-1">Key Specifications:</p>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(item.specifications).slice(0, 3).map(([key, value]) => (
                    <span key={key} className="text-custom-xs bg-gray-1 px-2 py-1 rounded">
                      {key}: {value}
                    </span>
                  ))}
                </div>
