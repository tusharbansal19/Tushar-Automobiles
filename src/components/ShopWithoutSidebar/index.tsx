...
const ShopWithoutSidebar = () => {
  ...
  useEffect(() => {
    if (!initializedRef.current) {
      console.log('Initializing ShopWithoutSidebar - loading data');
      // Clear any existing filters to show all parts
  ...
  useEffect(() => {
    if (pagination?.currentPage) {
      console.log('ShopWithoutSidebar - Page changed to:', pagination.currentPage, 'of', pagination.totalPages);
      console.log('ShopWithoutSidebar - Showing', displayedParts.length, 'items');
    }
  ...
  return (
    <>
      <Breadcrumb
        title={"Explore All Auto Parts"}
        pages={["shop", "/", "auto parts catalog"]}
      />
            ...
            <div className="w-full">
              ...
              {!loading && !error && displayedParts.length === 0 && allParts.length > 0 && (
                <div className="text-center py-20">
                  <p className="text-gray-5 text-lg">No auto parts found.</p>
                </div>
              )}
