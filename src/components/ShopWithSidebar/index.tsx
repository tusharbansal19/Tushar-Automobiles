...
const ShopWithSidebar = () => {
  ...
  // const searchParams = useSearchParams(); // Temporarily disabled
  ...
  const options = [
    { label: "Latest Parts", value: "createdAt-desc" },
  ...
  // Load data and filter options on mount (only once)
  useEffect(() => {
    if (!initializedRef.current) {
      console.log('Initializing ShopWithSidebar - loading data');
      dispatch(fetchFilterOptions());
      dispatch(fetchAllAutoParts());
      initializedRef.current = true;
    }
  }, [dispatch]);

  // Temporarily disabled URL parsing to fix infinite loop
  // useEffect(() => {
  //   // URL parsing logic will be added back later
  // }, []);



  // Debug pagination state changes (only log when page actually changes)
  useEffect(() => {
    if (pagination?.currentPage) {
      console.log('Page changed to:', pagination.currentPage, 'of', pagination.totalPages);
      console.log('Showing', displayedParts.length, 'items');
    }
  }, [pagination?.currentPage, pagination?.totalPages, displayedParts.length]);

  // No need for complex fetch logic - everything is handled in Redux
  ...
  const handleCompanyChange = (companies: string[]) => {
    dispatch(setCompanyFilter(companies));
  };

  // Handle vehicle type filter
  const handleVehicleTypeChange = (vehicleTypes: string[]) => {
    dispatch(setVehicleTypeFilter(vehicleTypes));
  };

  // Handle fuel type filter
  const handleFuelTypeChange = (fuelTypes: string[]) => {
    dispatch(setFuelTypeFilter(fuelTypes));
  };
  ...
  return (
    <>
      <Breadcrumb
        ...
        pages={["shop", "/", "automobile showroom"]}
      />
      <section className="overflow-hidden relative pb-20 pt-5 lg:pt-20 xl:pt-28 bg-[#f3f4f6]">
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="ax-w-[1170px] flex-col flex w-full mx-auto px-4 sm:px-8 xl:px-0">
            <div className="flex gap-7.5">
                ...
                <form onSubmit={(e) => e.preventDefault()}>
                  <div className="flex flex-col gap-6">
                    ...
                    <SearchFilter
                      ...
                      placeholder="Search by brand, model, part number..."
                    ...
                    {filterOptions?.vehicleTypes && (
                      <VehicleTypeFilter
                        ...
                        selectedVehicleTypes={filters.vehicleTypes}
                      />
                    )}
                    ...
                    {filterOptions?.categories && (
                      <CategoryDropdown
                        categories={filterOptions.categories.map(cat => ({
                          name: cat.name,
                          products: cat.count,
                          isRefined: filters.categories.includes(cat.name)
                        }))}
              ...
              {/* <!-- Content Start --> */}
              <div className="flex-2 w-full h-full bg-red-900 flex flex-col justify-evenly ">
                <div className="xl:max-w-[870px]  w-full">
                      ...
                      <div className="flex flex-wrap items-center gap-4">
                        ...
                        <p>
                          ...
                          {pagination && pagination.totalPages > 1 && (
                            <span className="text-gray-5"> (Page {pagination.currentPage} of {pagination.totalPages})</span>
                          )}
                        </p>
                      </div>
                  ...
                  {error && (
                    <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                      ...
                      <button
                        onClick={() => dispatch(fetchAllAutoParts())}
                  ...
                  {/* <!-- No Results --> */}
                  {!loading && !error && displayedParts.length === 0 && allParts.length > 0 && (
                    <div className="text-center py-20">
                      <p className="text-gray-5 text-lg">No auto parts found matching your criteria.</p>
                      <button
                        onClick={handleClearAllFilters}
                  ...
                  {/* <!-- No Data at All --> */}
                  {!loading && !error && allParts.length === 0 && (
                    <div className="text-center py-20">
                      <p className="text-gray-5 text-lg">No auto parts available.</p>
                    </div>
                  )}

                  {/* <!-- Products Grid Tab Content Start --> */}
                  {!loading && !error && displayedParts.length > 0 && (
                    <div
                      className={`${productStyle === "grid"
