...
interface CompanyFilterProps {
  companies: Company[];
  onCompanyChange: (selectedCompanies: string[]) => void;
}
...
interface CompanyFilterPropsExtended extends CompanyFilterProps {
  selectedCompanies?: string[];
}

const CompanyFilter = ({ companies, onCompanyChange, selectedCompanies = [] }: CompanyFilterPropsExtended) => {
  const [toggleDropdown, setToggleDropdown] = useState(true);

  const handleCompanyToggle = (companyName: string) => {
    const updatedSelection = selectedCompanies.includes(companyName)
      ? selectedCompanies.filter(name => name !== companyName)
      : [...selectedCompanies, companyName];
    
    onCompanyChange(updatedSelection);
  };
  ...
  return (
    <div className="bg-white shadow-1 rounded-lg">
      ...
      >
        ...
        <button
          aria-label="button for company dropdown"
